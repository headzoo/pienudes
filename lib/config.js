...
function merge(obj, def, path) {
    for (var key in def) {
        if (key in obj) {
            ...
            Logger.syslog.log("[WARNING] Missing config key " + (path + "." + key) + "; using default: " + JSON.stringify(def[key]));
            obj[key] = def[key];
        }
    }
}
...
exports.load = function (file) {
    ...
    if (cfg == null) {
        Logger.syslog.log(file + " is an Invalid configuration file, " + "assuming default configuration");
        cfg = defaults;
        return;
    }
...
function preprocessConfig(cfg) {
    ...
    if ("host" in cfg.http || "port" in cfg.http || "port" in cfg.https) {
        Logger.syslog.log("[WARN] The method of specifying which IP/port to bind has " + "changed.  The config loader will try to handle this " + "automatically, but you should read config.template.yaml " + "and change your config.yaml to the new format.");
        cfg.listen = [{
    ...
    // Setup nodemailer
    cfg.mail.nodemailer = nodemailer.createTransport(cfg.mail.config);
    ...
    for (var i = 0; i < cfg.listen.length; i++) {
        ...
        if (net.isIPv6(srv.ip) || srv.ip === "::") {
            if (srv.https && !cfg.io["ipv6-ssl"]) {
                if (!srv.url) {
                    Logger.errlog.log("Config Error: no URL defined for IPv6 " + "Socket.IO listener!  Ignoring this listener " + "because the Socket.IO client cannot connect to " + "a raw IPv6 address.");
    ...
    if (cfg["link-domain-blacklist"].length > 0) {
        cfg["link-domain-blacklist-regex"] = new RegExp(cfg["link-domain-blacklist"].join("|").replace(/\./g, "\\."), "gi");
    } else {
        // Match nothing
    ...
    if (cfg["youtube-v3-key"]) {
        require("cytube-mediaquery/lib/provider/youtube").setApiKey(cfg["youtube-v3-key"]);
    } else {
        Logger.errlog.log("Warning: No YouTube v3 API key set.  YouTube links will " + "not work.  See youtube-v3-key in config.template.yaml and " + "https://developers.google.com/youtube/registering_an_application for " + "information on registering an API key.");
    }
