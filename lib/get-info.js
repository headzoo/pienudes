...
var urlRetrieve = function urlRetrieve(transport, options, callback) {
    var req = transport.request(options, function (res) {
        res.on("error", function (err) {
            Logger.errlog.log("HTTP response " + options.host + options.path + " failed: " + err);
            callback(503, "");
        });
    ...
    req.on("error", function (err) {
        Logger.errlog.log("HTTP request " + options.host + options.path + " failed: " + err);
        callback(503, "");
    });
...
function convertMedia(media) {
    return new Media(media.id, media.title, media.duration, mediaTypeMap[media.type], media.meta);
}

var Getters = {
    /* youtube.com */
    yt: function yt(id, callback) {
        if (!Config.get("youtube-v3-key")) {
            return callback("The YouTube API now requires an API key.  Please see the " + "documentation for youtube-v3-key in config.template.yaml");
        }
    ...
    /* youtube.com search */
    ytSearch: function ytSearch(query, callback) {
        if (!Config.get("youtube-v3-key")) {
            return callback("The YouTube API now requires an API key.  Please see the " + "documentation for youtube-v3-key in config.template.yaml");
        }
    ...
    vi: function vi(id, callback) {
        ...
        Vimeo.lookup(id).then(function (video) {
    ...
    vi_oauth: function vi_oauth(id, callback) {
        var OAuth = require("oauth");
        var oa = new OAuth.OAuth("https://vimeo.com/oauth/request_token", "https://vimeo.com/oauth/access_token", Config.get("vimeo-oauth.consumer-key"), Config.get("vimeo-oauth.secret"), "1.0", null, "HMAC-SHA1");

        oa.get("https://vimeo.com/api/rest/v2?format=json" + "&method=vimeo.videos.getInfo&video_id=" + id, null, null, function (err, data, res) {
            if (err) {
    ...
    /* soundcloud.com */
    sc: function sc(id, callback) {
        /* TODO: require server owners to register their own API key, put in config */
        var SC_CLIENT = "2e0c82ab5a020f3a7509318146128abd";
...
module.exports = {
    Getters: Getters,
    getMedia: function getMedia(id, type, callback) {
